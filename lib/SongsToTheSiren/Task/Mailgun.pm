...
SongsToTheSiren::Task::Mailgun : Plugin to create routes that check auth

=head1 SYNOPSIS

    # In app startup()
    $self->plugin('SongsToTheSiren::Task::Mailgun');
...
Set via the environment, e.g.
    export songstothesiren_MAILGUN_FROM=noreply@example.com
...
Set via the environment, e.g.
    export songstothesiren_MAILGUN_DOMAIN=example.com
...
Set via the environment, e.g.
    export songstothesiren_MAILGUN_API_KEY=123ABCdef987
