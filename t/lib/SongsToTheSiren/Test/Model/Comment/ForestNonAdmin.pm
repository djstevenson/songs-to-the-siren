package SongsToTheSiren::Test::Model::Comment::ForestNonAdmin;
use Moose;
...
sub run {
	...
	subtest 'I add one comment, admin does not mod it, I should still see it' => sub {
		$first_root_comment = $self->_create_comment($user, $song, undef, 'BBCode1');

		my $forest = $song->get_comment_forest($user);
		is(scalar @$forest, 1, 'I see my own unmodded comments');

		$forest = $song->get_comment_forest($user2);
		is(scalar @$forest, 0, 'User2 does not see my unmodded comments');
	};
	...
	subtest 'Approve the single comment' => sub {
		...
		my $forest = $song->get_comment_forest($user);
		is(scalar @$forest, 1, 'I still see my comment after modding');
		is($forest->[0]->comment->id, $first_root_comment->id, 'Me: Root comment is right id');

		$forest = $song->get_comment_forest($user2);
		is(scalar @$forest, 1, 'User2 can now see my comment after modding');
		is($forest->[0]->comment->id, $first_root_comment->id, 'User2: Root comment is right id');
	};
...
sub _create_comment {
	...
	# Assumes the simplest of markdown, string with no bbcode in it..
