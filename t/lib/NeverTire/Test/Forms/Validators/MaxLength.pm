package NeverTire::Test::Forms::Validators::MaxLength;
use Moose;
...
use NeverTire::Form::Field::Validator::MaxLength;
...
sub run {
	...
	foreach my $test (@$tests){
		my $max = $test->{max};
		my $validator = NeverTire::Form::Field::Validator::MaxLength->new(max => $max, schema => $self->schema);
